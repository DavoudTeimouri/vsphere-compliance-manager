...
class PatternConfig(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    name = Column(String(200), nullable=False)
    pattern_type = Column(String(50))  # vm_name, datastore, role
...
class AnalysisFinding(Base):
    ...
    finding_type = Column(String(100))  # drs_violation, storage_shared, storage_scattered
...
class DRSRuleHistory(Base):
    ...
    action = Column(String(50))  # created, deleted, skipped
...
class StorageMoveHistory(Base):
    ...
    status = Column(String(50))  # proposed, approved, completed, failed
